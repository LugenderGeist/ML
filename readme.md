...
- **MAE (средняя абсолютная ошибка):** Средняя абсолютная величина ошибки в единицах целевой переменной. MAE = 5 означает, что модель ошибается в среднем на 5 смертей на 100 000 населения.
...
Наибольший вклад в модель линейной регрессии вносят медицинские показатели: «ср. кол-во рака в г.» (21.60%) и «ср. кол-во смертей в г.» (17.86%). Социально-экономические факторы («занятость_доход», «образование_доход», «% бедности») также играют значительную роль, но их суммарный вклад составляет около 25%.

| № | Признак | Коэффициент | Вклад |
|---|---------|-------------|-------|
| 1 | ср. кол-во рака в г. | +80.80 | 21.60% |
| 2 | ср. кол-во смертей в г. | -66.83 | 17.86% |
| 3 | смерти_на_диагнозы | +40.14 | 10.73% |
| 4 | занятость_доход | +28.55 | 7.63% |
| 5 | образование_доход | -28.51 | 7.62% |
| 6 | высшее_образование_на_бедность | +17.40 | 4.65% |
| 7 | ср. кол-во диагнозов/д.н. | +17.31 | 4.63% |
| 8 | % бедности | +13.96 | 3.73% |
| 9 | Население графства | -11.47 | 3.07% |
| 10 | % >16 работают | -9.91 | 2.65% |
...
Два ключевых признака по результатам построения дерева решений — «смерти_на_диагнозы» (57.34%) и «ср. кол-во диагнозов/д.н.» (38.97%) — в сумме они составляют более 96% важности, что указывает на их доминирующее влияние на прогнозируемый показатель смертности от рака.

| Признак | Важность |
|---------|----------|
| смерти_на_диагнозы | 57.34% |
| ср. кол-во диагнозов/д.н. | 38.97% |
| % >16 не работают | 1.19% |
| высшее_образование_на_бедность | 0.50% |
| ср. кол-во рака в г. | 0.44% |
| ср. кол-во смертей в г. | 0.42% |
| без_страховки | 0.42% |
| уровень_занятости | 0.21% |
| ср. возраст в округе | 0.14% |
| % 18-24 оконч. 11 классов | 0.11% |

### XGBoost
XGBoost — это высокоэффективная реализация градиентного бустинга. Алгоритм строит ансамбль деревьев последовательно, используя регуляризацию L1 и L2 для контроля сложности модели, а также эффективные параллельные вычисления и оптимизированную работу с разреженными матрицами. XGBoost требует более тщательной настройки гиперпараметров по сравнению с CatBoost, но при правильном подборе параметров часто демонстрирует лучшее качество среди всех градиентных бустингов.
...
- dense_1 — веса первого скрытого слоя (192 нейрона)
- dense_2 — веса второго скрытого слоя (48 нейрона)
- dense_3 — веса третьего скрытого слоя (64 нейронов)
...
- Увеличение числа эпох в нейронной сети приводило к ухудшению предсказаний. Небольшое увеличение шага спуска learning_rate (изначально стояло 0.01, увеличено до 0.03) увеличило точность. При большем шаге точность также падает.
...
В результате выполнения работы были получены навыки предобработки данных, построения различных моделей для предсказания целевой переменной. Были получены навыки работы с DVC.
